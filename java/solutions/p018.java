...
class problem18 {
	...
	public static void main(String... ignored) {
		...
		try (Stream<String> stream = Files.lines(Paths.get("C:\\Users\\Craig\\OneDrive - University Of Cambridge\\Code\\Java\\ProjectEuler\\src\\solutions.problem18.txt"))) {
	...
	public static void printArr() {
		for (int row=0;row<triangle.length;row++) {
			for (int col=0;col < triangle[0].length && triangle[row][col] != -1; col++) {
				String n = "";
				if      (triangle[row][col]/10 == 0) n="00";
				else if (triangle[row][col]/100== 0) n="0";
